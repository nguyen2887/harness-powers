...
- **Re-running refreshes harness-powers-owned artifacts in place** — the `HARNESS-POWERS` blocks in `CLAUDE.md`/`AGENTS.md` (only the marked block; your surrounding content is preserved), the vendored `.codex/.agents` skills, and the gate script — so a plugin upgrade lands without deleting anything by hand. Your own files (code, docs, `harness-powers.toml`, `harness.db`) are never overwritten. Existing hook-config JSON is left as is (merge the gate hook by hand if it's new to that file).
