...
**Announce at start:** "Classifying this task via harness intake before any other action."

`harness-cli` below means `scripts/bin/harness-cli` (macOS/Linux) or `.\scripts\bin\harness-cli.exe` (Windows).

## Checklist

Work through these in order (use a todo list if your CLI supports one):
...
6. **Announce and route** — state `Lane: <lane>. Reason: <flags/rationale>.` then continue with the next skill (see Routing).
...
Skills live in this same directory (`intake`, `designing`, `implementing`, `debugging`, `done`). Continue with the one your lane points to. If your CLI has not auto-loaded it, follow its steps from the file plus the AGENTS.md pipeline block.

- **Bug, test failure, or unexpected behavior** → record intake (usually `change-request`), then continue with the **debugging** skill
- **tiny** → continue with the **implementing** skill directly (no story packet; intake record is the only overhead)
- **normal / high-risk** → continue with the **designing** skill
