...
2. **Code-review gate** — the diff is reviewed in a **separate, human-driven reviewer pane, never a sub-invocation you launch**.
   ...
   - **Triage** — verify each finding technically before acting: Critical/Important → fix (re-enter the TDD loop for behavior changes), then re-review. Minor → fix or reject with a one-line technical reason.
